...
import utez.edu.mx.unidad3.modules.clients.ClientRepository;
...
public class CedeService {
    ...
    public APIResponse findById(long id) {
        try {
            ...
            if (found == null) {
                return new APIResponse("La cede no existe", null, false, HttpStatus.NOT_FOUND);
            }

            return new APIResponse("Operación existos", found, true, HttpStatus.OK);
        }catch (Exception ex) {
            ex.printStackTrace();
    ...
    public APIResponse saveCede(Cede payload) {
        try {
            ...
            //Necesitamos generar la clave de la Cede
            ...
            ex.printStackTrace();
            return new APIResponse("Error al guardar la cede", true, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}
